...
# ctb0703 is Volume 4 of the RADAMBRASIL project, started in 1970. ctb0703 was 
# published in 1974. The sampling year is missing in the source document. We set
# data_ano to 1972.
target_year <- 1972
br_soil2023[
  dataset_id == "ctb0703" & is.na(data_ano),
  `:=`(data_ano = target_year, data_ano_fonte = "estimativa")
]
br_soil2023[
  dataset_id == "ctb0703" & !is.na(data_ano) & is.na(data_ano_fonte),
  data_ano_fonte := "original"
]
summary_date(br_soil2023)
# Date: 10932 (yes) / 3104 (no)

# ctb0704 is Volume 2 of the RADAMBRASIL project, started in 1970. ctb0704 was
# published in 1973. The sampling year is missing in the source document. We set
# data_ano to 1971.
target_year <- 1971
br_soil2023[
  dataset_id == "ctb0704" & is.na(data_ano),
  `:=`(data_ano = target_year, data_ano_fonte = "estimativa")
]
br_soil2023[
  dataset_id == "ctb0704" & !is.na(data_ano) & is.na(data_ano_fonte),
  data_ano_fonte := "original"
]
summary_date(br_soil2023)
# Date: 10962 (yes) / 3074 (no)

# ctb0705 is Volume 5 of the RADAMBRASIL project, started in 1970. ctb0705 was
# published in 1974. The sampling year is missing in the source document. We set
# data_ano to 1972.
target_year <- 1972
br_soil2023[
  dataset_id == "ctb0705" & is.na(data_ano),
  `:=`(data_ano = target_year, data_ano_fonte = "estimativa")
]
br_soil2023[
  dataset_id == "ctb0705" & !is.na(data_ano) & is.na(data_ano_fonte),
  data_ano_fonte := "original"
]
summary_date(br_soil2023)
# Date: 10993 (yes) / 3043 (no)

# ctb0706 is Volume 6 of the RADAMBRASIL project, started in 1970. ctb0706 was
# published in 1974. The sampling year is missing in the source document. We set
# data_ano to 1972.
target_year <- 1972
br_soil2023[
  dataset_id == "ctb0706" & is.na(data_ano),
  `:=`(data_ano = target_year, data_ano_fonte = "estimativa")
]
br_soil2023[
  dataset_id == "ctb0706" & !is.na(data_ano) & is.na(data_ano_fonte),
  data_ano_fonte := "original"
]
summary_date(br_soil2023)
# Date: 11020 (yes) / 3016 (no)

# ctb0707 is Volume 7 of the RADAMBRASIL project, started in 1970. ctb0707 was
# published in 1975. The sampling year is missing in the source document. We set
# data_ano to 1973.
target_year <- 1973
br_soil2023[
  dataset_id == "ctb0707" & is.na(data_ano),
  `:=`(data_ano = target_year, data_ano_fonte = "estimativa")
]
br_soil2023[
  dataset_id == "ctb0707" & !is.na(data_ano) & is.na(data_ano_fonte),
  data_ano_fonte := "original"
]
summary_date(br_soil2023)
# Date: 11069 (yes) / 2967 (no)

# ctb0709 is Volume 8 of the RADAMBRASIL project, started in 1970. ctb0709 was
# published in 1975. The sampling year is missing in the source document. We set
# data_ano to 1973.
target_year <- 1973
br_soil2023[
  dataset_id == "ctb0709" & is.na(data_ano),
  `:=`(data_ano = target_year, data_ano_fonte = "estimativa")
]
br_soil2023[
  dataset_id == "ctb0709" & !is.na(data_ano) & is.na(data_ano_fonte),
  data_ano_fonte := "original"
]
summary_date(br_soil2023)
# Date: 11112 (yes) / 2924 (no)

# ctb0710 is Volume 9 of the RADAMBRASIL project, started in 1970. ctb0710 was
# published in 1975. The sampling year is missing in the source document. We set
# data_ano to 1973.
target_year <- 1973
br_soil2023[
  dataset_id == "ctb0710" & is.na(data_ano),
  `:=`(data_ano = target_year, data_ano_fonte = "estimativa")
]
br_soil2023[
  dataset_id == "ctb0710" & !is.na(data_ano) & is.na(data_ano_fonte),
  data_ano_fonte := "original"
]
summary_date(br_soil2023)
# Date: 11131 (yes) / 2905 (no)

# ctb0711 is Volume 10 of the RADAMBRASIL project, started in 1970. ctb0711 was
# published in 1976. The sampling year is missing in the source document. We set
# data_ano to 1974.
target_year <- 1974
br_soil2023[
  dataset_id == "ctb0711" & is.na(data_ano),
  `:=`(data_ano = target_year, data_ano_fonte = "estimativa")
]
br_soil2023[
  dataset_id == "ctb0711" & !is.na(data_ano) & is.na(data_ano_fonte),
  data_ano_fonte := "original"
]
summary_date(br_soil2023)
# Date: 11169 (yes) / 2867 (no)

# ctb0713 is Volume 12 of the RADAMBRASIL project, started in 1970. ctb0713 was
# published in 1976. The sampling year is missing in the source document. We set
# data_ano to 1974.
target_year <- 1974
br_soil2023[
  dataset_id == "ctb0713" & is.na(data_ano),
  `:=`(data_ano = target_year, data_ano_fonte = "estimativa")
]
br_soil2023[
  dataset_id == "ctb0713" & !is.na(data_ano) & is.na(data_ano_fonte),
  data_ano_fonte := "original"
]
summary_date(br_soil2023)
...
# RADAMBRASIL: set sampling year to year_min if still missing after the above
# operations. This is done because all sampling occurred before 1985, which is 
# the earliest year modeled by the MapBiomas Soil project. Although the 
# Brazilian Soil Dataset is not directly defined by the MapBiomas Soil project, 
# this adjustment is necessary due to dependencies in data processing. A more 
# accurate sampling date will be determined or estimated in the future.
idx <- br_soil2023[
...
summary_date(br_soil2023)
# Date: 13050 (yes) / 879 (no)

# Set sampling year to year_min for the following datasets:
# (we checked the source document and found that the sampling year is < 1985; 
# similar to RADAMBRASIL -- a proper solution will be determined in the future)
...
summary_date(br_soil2023)
# Date: 13084 (yes) / 845 (no)

# Use the average sampling date of the source soil survey for the following 
# datasets:
ctb <- c(
  "ctb0030", "ctb0032", "ctb0570", "ctb0572", "ctb0574", "ctb0617", "ctb0631",
  "ctb0639", "ctb0642", "ctb0645", "ctb0656", "ctb0657", "ctb0663", "ctb0667", 
  "ctb0668", "ctb0672", "ctb0673", "ctb0674", "ctb0675", "ctb0677", "ctb0679", 
  "ctb0684", "ctb0686", "ctb0691", "ctb0694", "ctb0700", "ctb0750", "ctb0774", 
  "ctb0775", "ctb0777", "ctb0781", "ctb0795", "ctb0808", "ctb0809", "ctb0811", 
  "ctb0820", "ctb0821", "ctb0822", "ctb0826", "ctb0831", "ctb0832"
)
...
# Set sampling_year to the average sampling year and data_ano_fonte to 
# "estimativa"
br_soil2023[
...
summary_date(br_soil2023)
# Date: 13863 (yes) / 66 (no)

# ctb0009
# Events in ctb0009 missing the sampling date are from ctb0003, thus they are
# already included in the dataset. Thus, they can be removed to avoid 
# duplicates.
ctb <- "ctb0009"
br_soil2023 <- br_soil2023[!(dataset_id %in% ctb & is.na(data_ano)), ]
summary_date(br_soil2023)
# Date: 13863 (yes) / 34 (no)

# ctb0029
# Events in ctb0029 where municipio_id %in% c("Santa Maria", "Itaara"), 
# amostra_tipo == "COMPOSTA", amostra_quanti == 3, and data_ano == 2009 are
# from ctb0003, thus they are already included in the dataset and can be 
...
summary_date(br_soil2023)
# Date: 13825 (yes) / 34 (no)
# All of the remain 34 events without sampling date are from ctb0029. Soil
# samples were taken around 2009, but the exact date is not known. We set it to
# 2009 as a reasonable estimate.
target_year <- 2009
...
# FIGURE. Temporal distribution of samples with known sampling date after data
# rescue and estimation.
missing_time <- is.na(br_soil2023[["data_ano"]])
# Plot histogram
file_path <- fig_path("103_temporal_distribution_after_estimation.png")
png(file_path, width = 8, height = 5, units = "in", res = 300)
x <- br_soil2023[, data_ano[1], by = c("dataset_id", "observacao_id")][, V1]
hist(x,
  xlab = "Year",
  ylab = "Frequency",
  main = "Temporal distribution of events after data rescue and estimation",
